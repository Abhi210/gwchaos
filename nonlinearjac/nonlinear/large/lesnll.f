C *** THIS IS WITH NEW ERROR CONTROL NORMS.  April 23, 2004. ***
C *** THIS IS ALSO WITH NEW CHANGE WITH IPAR(10)  *** April 26, 2004
C *** ALSO CHANGE IN CASE OF SUCCESSFUL STEP  *** April 27, 2004
...
C *** ALSO WITH DIFFERENT TOLS FOR TRAJECTORY, Q, LEs *** LD, May 29 2004 


      SUBROUTINE LESNLL(GETF,GETDFV,M,N,APPLES,T0,TE,DT,Y0,X0,TOLT,
     *                  TOLQ,TOLL,IPAR,FWORK,IFLAG,INARR,REARR)

C
C ***** WARNING: before calling LESNLL for the very first time,
...
C                done *****
C
C	LESNLL approximates N Lyapunov exponents of a 
C       nonlinear system y'=f(y) on the interval [T0,TE]
C
C	LESNLL refers to: Lyapunov ExponentS NonLinear Large
C       in that it approximates Lyapunov exponents of a 
C       nonlinear system whose Jacobian matrix Df(y(t)) is not
C       explicitly required.  All that is needed is the action of
C       a Jacobian on a vector.  The user can specify this action
C       in a subroutine (see GETDFV below) or the code can compute
C       it internally without user's intervention (see IPAR(7)).
C       In case of small systems, for which the Jacobian
C       matrix can be explicitely computed and stored, the user
C       should consider using LESNLS (Lyapunov Exponents
C       NonLinear Small).  For linear systems, see the
C       documentations of LESLIS and LESLIL. 
C
C	LESNLL is an interface to subroutines which approximate
...
C       LESNLL can be used also just to approximate the solution
...
C              program and the routines GETF and GETDF.  Treat them as dummy
C              arguments if they are not needed.
C
C       GETDFV - subroutine that returns the Jacobian times a vector.
C           You must declare the variable GETDFV as EXTERNAL, though
C           you may avoid specifying the routine (see IPAR(7)), 
C           in which case treat the argument as dummy.
C           But, if you specify the routine, then it must
C           have parameter list of the form
C                       GETDFV(M,Y,V,DFV,INARR,REARR)
C           where M is the dimension of the matrix,
C           Y are the M dependent variables and 
C           DFV is the product of the Jacobian matrix Df times the vector v
C           INARR and REARR are integer and double precision arrays
C              that the user can use for communication between the calling
...
C                        the user after a successful step in the direction of TE
C	       IPAR(3) - INPUT: must be set to 1 for every "new" call to the code
C                       (in one step mode or otherwise, and in constant 
C                       or variable stepsize mode)
C                       A "new" call to the code is one for
C                       which values of any of IPAR(1,2,6,7,9,10) have been
...
C                        IPAR(5).GE.11*M*N+14*M+8*N+63
...
C                        [Use of IPAR(6)=1 is justified if the user wants to ensure
...
C              IPAR(7) - INPUT: Specifies if the action of the Jacobian on a vector, 
C                        Df*v, is user specified in GETDFV, or must be internally 
C                        approximated
C                        It is usually simpler to let the code approximate Df*v, but
C                        it may be more accurate or even efficient to specify it 
C                        if a simple expression exists for Df*v.
C                        It must be appreciated that LESNLL "approximates" Df*v by
C                        using numerical directional derivatives.  As such, this
C                        approximation will generally be only accurate to the square 
C                        root of machine precision
C                        IPAR(7)=0 (DEFAULT) means that the action of the Jacobian
C                           on a vector is approximated internally by the code
C                        IPAR(7)=1 means that the user specified the action of the 
C                           Jacobian on a vector in GETDFV (see specifications)
C                        If user only want trajectory and have set IPAR(6)=1, then 
C                        IPAR(7) and indeed the action Df*v are irrelevant.  
...
C                        Of course, if IPAR(6)=1, only solution trajectory is
...
CCCCCCC POINTERS, AND COMMON VARIABLES
      ...
      INTEGER INTDIR, IFIRST, IWHAT, IJAC
      INTEGER IXPTR, IXHPTR, IQPTR, IBPTR
      ...
      INTEGER K1YPTR, K2YPTR, K3YPTR, K4YPTR, K5YPTR, K6YPTR, K7YPTR
      INTEGER I1YPTR, IS1YPT, IS2YPT, IS3YPT, IS4YPT
      COMMON /STEP/ HMIN, HNEXT, TFIRST, 
     *              INTDIR, IFIRST, IWHAT, IJAC, LAST
      SAVE /STEP/
      COMMON /NSQUAX/ IXPTR, IQPTR, IXHPTR, IBPTR
      SAVE /NSQUAX/
     ...
     *                K7YPTR, I1YPTR, IS1YPT, IS2YPT, IS3YPT, IS4YPT
      SAVE /YRKPTR/
...
C IN NEWYES WE CHECK/DEFAULT ALL THINGS TO BE DONE ON A NEW CALL
      CALL NEWYES(GETF,GETDFV,M,N,APPLES,T0,TE,DT,Y0,X0,TOLT,TOLQ,
...
C ADJUST STEPSIZE AS NEEDED, AND STORE STEP WHICH WOULD HAVE BEEN USED
      IF (LAST) THEN
         LAST=.FALSE.
         DT=HNEXT
      ENDIF
...
C CALL TO ONE OF THE DIFFERENT METHODS.
      IF (IPAR(8).EQ.0) THEN 
CCCCCCC PQDPNS
         CALL PQDPNL(GETF,GETDFV,M,N,T,DT,HOLD,Y0,X0,APPLES,TOLT,
     *    TOLQ,TOLL,FWORK(IQPTR),FWORK(IXHPTR),
     ...
     *    FWORK(K7YPTR), FWORK(I1YPTR), FWORK(IS1YPT), FWORK(IS2YPT),  
     *    FWORK(IS3YPT), FWORK(IS4YPT), IPAR,NREJ,IFLAG,INARR,REARR)
       ELSEIF (IPAR(8).EQ.1) THEN
CCCCCCC HQDPNL
         CALL HQDPNL(GETF,GETDFV,M,N,T,DT,HOLD,Y0,X0,APPLES,TOLT,
     *    TOLQ,TOLL,FWORK(IXPTR),FWORK(IXHPTR),FWORK(IQPTR), 
     *    FWORK(IBPTR),FWORK(K1PTR),
     *    FWORK(K2PTR), FWORK(K3PTR),FWORK(K4PTR),FWORK(K5PTR),
     *    FWORK(K6PTR), FWORK(ISTAGE),
     *    FWORK(INUPTR),FWORK(INUHTP),FWORK(K1NUPT),FWORK(K2NUPT), 
...
CCCCCCC PQ38NL
         CALL PQ38NL(GETF,GETDFV,M,N,T,DT,HOLD,Y0,X0,APPLES,TOLT,
     *    TOLQ,TOLL,FWORK(IQPTR),FWORK(IXHPTR), 
     *    FWORK(IBPTR),FWORK(K1PTR),
     ...
     *    FWORK(IARKPT),FWORK(IBHTPT),
     *    FWORK(IYPTR), FWORK(IYHTPT), FWORK(K1YPTR), FWORK(K2YPTR),
     *    FWORK(K3YPTR), FWORK(K4YPTR), FWORK(K5YPTR), FWORK(I1YPTR), 
     *    FWORK(IS1YPT), FWORK(IS2YPT), IPAR,NREJ,IFLAG,INARR,REARR)
       ELSEIF (IPAR(8).EQ.3) THEN
CCCCCCC HQ38NL
         CALL HQ38NL(GETF,GETDFV,M,N,T,DT,HOLD,Y0,X0,APPLES,TOLT,
     *    TOLQ,TOLL,FWORK(IXPTR),FWORK(IXHPTR), FWORK(IQPTR), 
     *    FWORK(IBPTR),FWORK(K1PTR),
     *    FWORK(K2PTR), FWORK(K3PTR),FWORK(K4PTR),FWORK(ISTAGE),
     *    FWORK(INUPTR),FWORK(INUHTP),FWORK(K1NUPT),FWORK(K2NUPT), 
...
CCCCCCC XDPNL
         CALL XDPNL(GETF,GETDFV,M,N,T,DT,HOLD,Y0,X0,APPLES,TOLT,
     *    TOLL,FWORK(IXPTR),FWORK(IXHPTR),FWORK(K1PTR),
     *    FWORK(K2PTR), FWORK(K3PTR),FWORK(K4PTR),FWORK(K5PTR),
     *    FWORK(K6PTR), FWORK(ISTAGE),
     *    FWORK(IARKPT),FWORK(IBHTPT),
     *    FWORK(IYPTR), FWORK(IYHTPT), FWORK(K1YPTR), FWORK(K2YPTR),
     *    FWORK(K3YPTR), FWORK(K4YPTR), FWORK(K5YPTR), FWORK(K6YPTR), 
     *    FWORK(K7YPTR), FWORK(I1YPTR), FWORK(IS1YPT), FWORK(IS2YPT), 
     *    FWORK(IS3YPT), FWORK(IS4YPT), IPAR(1), IPAR(10),
     *    NREJ,IFLAG,INARR,REARR)
       ELSEIF (IPAR(8).EQ.5) THEN
CCCCCCC X38NL
         CALL X38NL(GETF,GETDFV,M,N,T,DT,HOLD,Y0,X0,APPLES,TOLT,
     *    TOLL,FWORK(IXPTR),FWORK(IXHPTR),FWORK(K1PTR),
     *    FWORK(K2PTR), FWORK(K3PTR),FWORK(K4PTR),FWORK(ISTAGE),
     *    FWORK(IARKPT),FWORK(IBHTPT),
     *    FWORK(IYPTR), FWORK(IYHTPT), FWORK(K1YPTR), FWORK(K2YPTR),
     *    FWORK(K3YPTR), FWORK(K4YPTR), FWORK(K5YPTR), FWORK(I1YPTR), 
     *    FWORK(IS1YPT), FWORK(IS2YPT), IPAR(1),IPAR(10),NREJ,IFLAG,
     *    INARR,REARR)
      ENDIF
...
C CHECK IF FAILED ON WHAT HAD TO BE LAST STEP 
      IF((LAST).AND.(DABS(T).LT.DABS(TE))) LAST=.FALSE.
...
CCCCCCC RETURN IF IN ONESTEP MODE 
      IF ((IPAR(2).EQ.0).AND.(INTDIR*T.LT.INTDIR*TE)) GOTO 20 

      RETURN
      END
CCCCC END OF LESNLL CCCCC
...
C CHECK CORRECTENESS OF INPUT ON M AND N
      ...
      IF ((N.GT.M).OR.(N.LT.1)) THEN
         ...
         RETURN
      END IF
C CHECK VALUES OF IPAR(3,4,5,8) AND SET DEFAULTS IF NEEDED
      IF (IPAR(3).NE.1) THEN
         IFLAG=2
         RETURN
      END IF
...
CC       CURRENTLY NEED DIM(FWORK).GE.11*M*N+14*M+8*N+63
      IF (IPAR(5).LT.(11*M*N+14*M+8*N+63)) THEN
         IFLAG=1
         RETURN
      END IF
...
C  TRANSITIONS X AND XHAT,  WORKSPACE QMAT AND BMAT IN /NSQUAX/  
      IXPTR=1        
      IQPTR=IXPTR+M*N
      IXHPTR = IQPTR+M*N
      IBPTR=IXHPTR+M*N
...
CCCCCC NEWYES CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      SUBROUTINE NEWYES(GETF,GETDFV,M,N,APPLES,T0,TE,DT,Y0,X0,TOLT,
     *                  TOLQ,TOLL,IPAR,FWORK,IFLAG,INARR,REARR)
...
C MONITOR/DEFAULT IF HAVE JACOBIAN*VECTOR
      IF (IPAR(7).NE.1) IPAR(7)=0
      IJAC=IPAR(7)
...
C QDOT RETURNS QDOT IN RK1
                  CALL QDOT(GETF,GETDFV,M,N,Y0,FWORK(K1YPTR),
     1                FWORK(IBPTR),X0,FWORK(K1PTR),FWORK(K2YPTR),
     2                IJAC,INARR,REARR)
              ELSE
C XDOT RETURNS XDOT IN RK1
                  CALL XDOT(GETF,GETDFV,M,N,Y0,FWORK(K1YPTR),
     1                   X0,FWORK(K1PTR),FWORK(K2YPTR),
     2                   IJAC,INARR,REARR)
            ENDIF
...
C
C input: 
C     GETF, declared EXTERNAL, returns the vector field
C     GETDFV, declared EXTERNAL, returns Jacobian times a vector
C             if user specified it.  Ignored otherwise
C     M, dimension of the coefficient matrix (i.e., of vector field)
C     N, number of columns of Q (i.e., LEs to be found)
C     Y0, value where to get derivative (Jacobian)
C     FY0, the vector field at Y0
C     B, workspace
C     QMAT, the matrix Q
C     IJAC, specifies if user supplied or not Df*v
C     WORK, vector of workspace to avoid overwriting important
C           things needed later
C     INARR and REARR, communication arrays with user's routine
C output:
C     B, on diagonal it has diag(Q^T*Jac*Q)
C     QP, the first derivative of Q
C
C BLAS: Blas3: DGEMM
C
C*****Authors: L Dieci
C*****Date: April 28, 2004
C
CCCCCC QDOT CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      SUBROUTINE QDOT(GETF,GETDFV,M,N,Y0,FY0,B,QMAT,QP,WORK,
     1                IJAC,INARR,REARR)
      IMPLICIT NONE

      EXTERNAL GETF, GETDFV
      INTEGER M, N, IJAC, INARR(*)
      DOUBLE PRECISION Y0(M),FY0(M), REARR(*)
      DOUBLE PRECISION B(M,N),QMAT(M,N),QP(M,N), WORK(M)
...
C FORM Jacobian*Q
 ...
 2          QP(I,J) = ZERO
         IF (IJAC.EQ.0) THEN
            CALL OURDFV(GETF,M,Y0,FY0,WORK,QP(1,J),INARR,REARR) 
           ELSE
            CALL GETDFV(M,Y0,WORK,QP(1,J),INARR,REARR) 
         ENDIF
 7    CONTINUE
C FORM Q^T*A*Q-S (S IS SKEW MATRIX)
      CALL DGEMM ('T', 'N', N, N, M, ONE, QMAT, M, QP, M,
     $                   ZERO, B, M )
C FINISH OFF Q^T*A*Q-S
 ...
 11   CONTINUE
C UPDATE QDOT
      CALL DGEMM ('N', 'N', M, N, N, -1.0D0*ONE, QMAT, M, B, M,
     $                   ONE, QP, M )
...
C     M, dimension of the coefficient matrix (i.e., of vector field)
C     N, number of columns of Q (i.e., LEs to be found)
C     Y0, argument at which we need to evaluate Jacobian
C     FY0, the vector field at Y0
C     XMAT, the initial value (usually, the matrix Q)
C     IJAC, specifies if user supplied or not Df*v
C     WORK, vector of workspace to avoid overwriting important
C           things needed later
C     INARR and REARR, communication arrays with user's routine
...
CCCCCC XDOT CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      SUBROUTINE XDOT(GETF,GETDFV,M,N,Y0,FY0,XMAT,XP,
     1                WORK,IJAC,INARR,REARR)
      IMPLICIT NONE

      EXTERNAL GETF, GETDFV
      INTEGER M, N, IJAC, INARR(*)
      DOUBLE PRECISION Y0(M), FY0(M), REARR(*)
      DOUBLE PRECISION XMAT(M,N), XP(M,N), WORK(M)
...
C FORM Jacobian*X
 ...
 2          XP(I,J) = ZERO
         IF (IJAC.EQ.0) THEN
            CALL OURDFV(GETF,M,Y0,FY0,WORK,XP(1,J),INARR,REARR) 
           ELSE
            CALL GETDFV(M,Y0,WORK,XP(1,J),INARR,REARR) 
         ENDIF
 7    CONTINUE
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C
C*****Subroutine name: OURDFV
C
C*****Purpose
C
C  returns the jacobian times a vector.  Jacobian is approximated
C  by divided difference for directional derivatives
C	
C input: 
C     GETF, declared EXTERNAL, returns vector field
C     M, dimension of the system (coefficient matrix)
C     Y, argument at which we need to evaluate Jacobian
C     FY, vector field evaluated at Y 
C     V, vector to which the Jacobian must be applied
C        CAREFUL: V is overwritten
C     INARR and REARR, communication arrays with user's routine
C output:
C     DFV, the Jacobian acting on the vector V
C
C*****Authors: L Dieci & ES Van Vleck
C*****Date: April 28, 2004
C
CCCCCC OURDFV CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      SUBROUTINE OURDFV(GETF,M,Y,FY,V,DFV,INARR,REARR)
      IMPLICIT NONE

      EXTERNAL GETF
      INTEGER M, INARR(*)
      DOUBLE PRECISION  Y(M), FY(M), V(M), DFV(M), REARR(*)

      DOUBLE PRECISION ROOTEP
      PARAMETER (ROOTEP=1.490116119384766D-8)
...
C FIND INCREMENT FOR DIVIDED DIFFERENCE MATRIX FREE
      CALL TNORM(M,FY,DH)
C      DH=DH/DSQRT(DFLOAT(M))
      DH=DMAX1(DH,ONE)
      DH=ROOTEP*DH
 ...
 5    CONTINUE
      ...
      RETURN
      END
CCCCC END OF OURDFV CCCCC
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C
C*****Subroutine name: PQDPNL
C
C*****Purpose
C
...
C trajectory, and/or (if required) on the LEs and/or Q.
...
C
C       This is for "large" problems, whereby no explicit Jacobian
C          is ever formed.  Code uses routine "GETDFV" if user supplied
C          it, or the internal routine "ourdfv" depending on IJAC.
C
C          Code can proceed in fixed or variable
C          step size.  In the latter case, error control is 
C          performed on the trajectory, and/or on the Lyapunov exponents,
C          and/or on the Q-factor.
C
C*****Author: L Dieci 
C*****Date: April 28, 2004
C
CCCCCC PQDPNL CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      SUBROUTINE PQDPNL(GETF,GETDFV,M,N,T,H,HOLD,Y0,X0,RLYAP,TOLT,
     *          TOLQ,TOLL,Q,QHAT,BMAT,RK1,RK2,RK3,RK4,RK5,RK6,STAGE,  
     *          RNU,RNUHAT,RKNU1,RKNU2,RKNU3,RKNU4,RKNU5,RKNU6,
     *          ARK,BHAT,Y,YHAT,YRK1,YRK2,YRK3,YRK4,YRK5,YRK6,
     *          YRK7,Y1,Y15,Y03,Y45,Y89,
     *          IPAR,NREJ,IFLAG,INARR,REARR)
      ...
      DOUBLE PRECISION Q(M,N), QHAT(M,N)
      ...
      DOUBLE PRECISION Y(M),YHAT(M),YRK1(M),YRK2(M),YRK3(M)
      DOUBLE PRECISION YRK4(M),YRK5(M),YRK6(M),YRK7(M)
      DOUBLE PRECISION Y1(M),Y15(M),Y03(M),Y45(M),Y89(M)

      LOGICAL PREVIO, LAST
      INTEGER INTDIR, IFIRST, IFXDPF, IERRC, IHOWLE, IWHAT, IJAC
      INTEGER I, J
...
CCCCCCCCCC INTEGRATE FOR X: RETURNS X AND XHAT
      CALL PRK5NL(GETF,GETDFV,PREVIO,M,N,IFIRST,IWHAT,IJAC,
     *        IFXDPF,IHOWLE,IERRC,H,Y0,X0,
     *        Q,QHAT,BMAT,RK1,RK2,RK3,RK4,RK5,RK6,
     *        STAGE, RNU,RNUHAT,RKNU1,RKNU2,RKNU3,RKNU4,
     *        RKNU5,RKNU6,ARK,BHAT,
     *        Y,YHAT,YRK1,YRK2,YRK3,YRK4,YRK5,YRK6,YRK7,
...
C         RLYAP(I)=((T-TFIRST)*SCALE)*RLYAP(I)+SCALE*RNUHAT(I)
 ...
 32   CONTINUE
      ...
      RETURN
      END
CCCCC END OF PQDPNL CCCCC
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C
C*****Subroutine name: PRK5NL
C
C*****Purpose
C
...
C*****Date: April 28, 2004
C
CCCCCC PRK5NL CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      SUBROUTINE PRK5NL(GETF,GETDFV,PREVIO,M,N,IFIRST,IWHAT,IJAC,
     *        IFXDPF,IHOWLE,IERRC,H,Y0,X0,
     *        Q,QHAT,BMAT,RK1,RK2,RK3,RK4,RK5,RK6,
     *        STAGE, RNU,RNUHAT,RKNU1,RKNU2,RKNU3,RKNU4,
     *        RKNU5,RKNU6,ARK,BHAT,
     ...
     *        Y1,Y15,Y03,Y45,Y89,IFLAG,INARR,REARR)
      ...
      EXTERNAL GETF,GETDFV
      LOGICAL PREVIO
      INTEGER M, N, IFIRST, IWHAT, IJAC,IFXDPF, IHOWLE
      ...
      DOUBLE PRECISION YRK5(M),YRK6(M),YRK7(M)
      DOUBLE PRECISION Y1(M),Y15(M),Y03(M),Y45(M),Y89(M)
      DOUBLE PRECISION ONE, ZERO, EXP1,EXP2
      DOUBLE PRECISION HINCR, HSFTY, HDRSTC,TEPS
      INTEGER I, J
...
C      IF (IFIRST.EQ.1) THEN
      IF ((IFIRST.EQ.1).OR.(IFXDPF.EQ.1)) THEN
         ...
         GOTO 7
      END IF
C IF NOT FAILED PREVIOUS STEP, YRK IS IN YRK7
...
C ... RK7, AND COMPARISON SOLUTION XHAT ...
C TO BE USED FOR ERROR MONITOR/STEP-SIZE SELECTION
      CALL GETF(M,Y,YRK7,INARR,REARR) 
      DO 24 I=1,M 
...
CC **** SEE IF CAN ECONOMIZE HERE ****
      CALL QDOT(GETF,GETDFV,M,N,Y0,YRK1,BMAT,Q,RK1,QHAT(1,1),
     1          IJAC,INARR,REARR)
      DO 29 J=1,N
         DO 29 I=1,M
 ...
 30      RKNU1(I)=BMAT(I,I)
      ...
      ENDIF
C ... RK2 AND U03 ...
      CALL QDOT(GETF,GETDFV,M,N,Y15,YRK2,BMAT,STAGE,RK2,QHAT(1,1),
     1          IJAC,INARR,REARR)
      DO 34 J=1,N
         DO 34 I=1,M
 ...
 40         RKNU2(I)=BMAT(I,I)
      ...
      ENDIF
C ... RK3 AND U45 ...
      CALL QDOT(GETF,GETDFV,M,N,Y03,YRK3,BMAT,STAGE,RK3,QHAT(1,1),
     1          IJAC,INARR,REARR)
      DO 43 J=1,N
         DO 43 I=1,M
 ...
 50         RKNU3(I)=BMAT(I,I)
      ...
      ENDIF
C ... RK4 AND U89 
      CALL QDOT(GETF,GETDFV,M,N,Y45,YRK4,BMAT,STAGE,RK4,QHAT(1,1),
     1          IJAC,INARR,REARR)
       DO 55 J=1,N
         DO 55 I=1,M
 ...
 60         RKNU4(I)=BMAT(I,I)
      ...
      ENDIF
C ... RK5 AND U1 (STAGE VALUE AT H) ...
      CALL QDOT(GETF,GETDFV,M,N,Y89,YRK5,BMAT,STAGE,RK5,QHAT(1,1),
     1          IJAC,INARR,REARR)
      DO 65 J=1,N
         DO 65 I=1,M
 ...
 70         RKNU5(I)=BMAT(I,I)
      ...
      ENDIF
C ... RK6, SOLUTION AT H ...
      CALL QDOT(GETF,GETDFV,M,N,Y1,YRK6,BMAT,STAGE,RK6,QHAT(1,1),
     1          IJAC,INARR,REARR)
      IF (IHOWLE.EQ.0) THEN
         DO 77 I=1,N
 77         RKNU6(I)=BMAT(I,I)
...
C TO BE USED FOR ERROR MONITOR/STEP-SIZE SELECTION
     ...
     *    (IERRC.EQ.210)).AND.IFXDPF.EQ.0) THEN
         CALL QDOT(GETF,GETDFV,M,N,Y,YRK7,BMAT,Q,RK2,QHAT(1,1),
     1          IJAC,INARR,REARR)
        ELSE
C FORM Jacobian*Q
         DO 83 J=1,N
...
C 82            STAGE(I,J) = ZERO
            IF (IJAC.EQ.0) THEN
               CALL OURDFV(GETF,M,Y,YRK7,RK2(1,J),STAGE(1,J),
     1                     INARR,REARR) 
              ELSE
               CALL GETDFV(M,Y,RK2(1,J),STAGE(1,J),INARR,REARR) 
            ENDIF
 83      CONTINUE
C GET diagonal of Q^T*(Jacobian*Q)
         CALL DGQTB(M,M,N,Q,STAGE,M,BMAT,M)
      ENDIF
      DO 86 I=1,N
 86      RKNU2(I)=BMAT(I,I)
      DO 90 J=1,N 
     ...
     *       IERRC.EQ.21.OR.IERRC.EQ.210)) THEN
            DO 88 I=1,M
               QHAT(I,J)=X0(I,J)+H*(BHAT(1)*RK1(I,J)+
 ...
 90   CONTINUE
     ...
     *    IERRC.EQ.21.OR.IERRC.EQ.210)) THEN
      ...
      RETURN
      END
CCCCC END OF PRK5NL CCCCC
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C
C*****Subroutine name: HQDPNL
C
C*****Purpose
C
...
C*****Date: May 29, 2003
C
CCCCCC HQDPNL CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      SUBROUTINE HQDPNL(GETF,GETDFV,M,N,T,H,HOLD,Y0,X0,RLYAP,TOLT,
     *          TOLQ,TOLL,X,XHAT,QMAT,BMAT,RK1,RK2,RK3,RK4,RK5,RK6,  
     *          STAGE,RNU,RNUHAT,RKNU1,RKNU2,RKNU3,RKNU4,RKNU5,RKNU6,
     *          ARK,BHAT,Y,YHAT,YRK1,YRK2,YRK3,YRK4,YRK5,YRK6,
     *          YRK7,Y1,Y15,Y03,Y45,Y89,
     *          IPAR,NREJ,IFLAG,INARR,REARR)
      ...
      DOUBLE PRECISION T,H,HOLD, Y0(M),REARR(*)
      DOUBLE PRECISION X0(M,N), RLYAP(N),TOLT,TOLQ, TOLL(*)
      DOUBLE PRECISION X(M,N), XHAT(M,N)
...
C OF TRANSITIONS.  RETURNS ALSO UPDATE FOR LES
      CALL HRK5NL(GETF,GETDFV,PREVIO,M,N,IFIRST,IWHAT,IJAC,
     *        IFXDPF,IHOWLE,IERRC,H,Y0,X0,
     *        X,XHAT,QMAT,BMAT,RK1,RK2,RK3,RK4,RK5,RK6,
     *        STAGE, RNU,RNUHAT,RKNU1,RKNU2,RKNU3,RKNU4,
     *        RKNU5,RKNU6,ARK,BHAT,
     *        Y,YHAT,YRK1,YRK2,YRK3,YRK4,YRK5,YRK6,YRK7,
     *        Y1,Y15,Y03,Y45,Y89,IFLAG,INARR,REARR)
...
C         RLYAP(I)=((T-TFIRST)*SCALE)*RLYAP(I)+SCALE*RNUHAT(I)
 ...
 32   CONTINUE
      ...
      RETURN
      END
CCCCC END OF HQDPNL CCCCC
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C
C*****Subroutine name: HRK5NL
C
C*****Purpose
C
C Integrates on one step the solution of x'=f(x).  If needed,
...
C*****Author: L Dieci 
C*****Date: April 28, 2004
CCCCCC HRK5NL CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      SUBROUTINE HRK5NL(GETF,GETDFV,PREVIO,M,N,IFIRST,IWHAT,IJAC,
     *        IFXDPF,IHOWLE,IERRC,H,Y0,X0,
     *        X,XHAT,QMAT,BMAT,RK1,RK2,RK3,RK4,RK5,RK6,
     *        STAGE, RNU,RNUHAT,RKNU1,RKNU2,RKNU3,RKNU4,
     *        RKNU5,RKNU6,ARK,BHAT,
...
C (THE LATTER IS NOT THE SOLUTION VALUE, BUT JUST THE STAGE VALUE)
C ... RK1 AND U15 ...
C VERY FIRST CALL OR IF NOT FAILED STEP NEED RK1
C OTHERWISE, HAVE RK1 IN RK1 
      IF ((IFIRST.EQ.1).OR.(.NOT.PREVIO)) THEN
         CALL XDOT(GETF,GETDFV,M,N,Y0,YRK1,X,RK1,XHAT(1,1),
     1             IJAC,INARR,REARR) 
      END IF
 ...
 30   CONTINUE
C ... RK2 AND U03 ...
      CALL XDOT(GETF,GETDFV,M,N,Y15,YRK2,STAGE,RK2,XHAT(1,1),
     1          IJAC,INARR,REARR) 
 ...
 32         QMAT(I,J)=STAGE(I,J)
      ...
      IF (IFLAG.NE.0) THEN
         ...
         RETURN
      ENDIF
      CALL XDOT(GETF,GETDFV,M,N,Y15,YRK2,QMAT,STAGE,XHAT(1,1),
     1          IJAC,INARR,REARR) 
      CALL DGQTB(M,M,N,QMAT,STAGE,M,BMAT,M)
      DO 34 I=1,N
 34      RKNU2(I)=BMAT(I,I)
 36   CONTINUE
      DO 40 J=1,N
 ...
 40   CONTINUE
C ... RK3 AND U45 ...
      CALL XDOT(GETF,GETDFV,M,N,Y03,YRK3,STAGE,RK3,XHAT(1,1),
     1          IJAC,INARR,REARR) 
 ...
 41         QMAT(I,J)=STAGE(I,J)
      ...
      IF (IFLAG.NE.0) THEN
         ...
         RETURN
      ENDIF
      CALL XDOT(GETF,GETDFV,M,N,Y03,YRK3,QMAT,STAGE,XHAT(1,1),
     1          IJAC,INARR,REARR) 
      CALL DGQTB(M,M,N,QMAT,STAGE,M,BMAT,M)
      DO 43 I=1,N
 43      RKNU3(I)=BMAT(I,I)
 ...
 50   CONTINUE
C ... RK4 AND U89 
      CALL XDOT(GETF,GETDFV,M,N,Y45,YRK4,STAGE,RK4,XHAT(1,1),
     1          IJAC,INARR,REARR) 
 ...
 52         QMAT(I,J)=STAGE(I,J)
      ...
      IF (IFLAG.NE.0) THEN
         ...
         RETURN
      ENDIF
      CALL XDOT(GETF,GETDFV,M,N,Y45,YRK4,QMAT,STAGE,XHAT(1,1),
     1          IJAC,INARR,REARR) 
      CALL DGQTB(M,M,N,QMAT,STAGE,M,BMAT,M)
      DO 55 I=1,N
 55      RKNU4(I)=BMAT(I,I)
 ...
 60   CONTINUE
C ... RK5 AND U1 (STAGE VALUE AT H) ...
      CALL XDOT(GETF,GETDFV,M,N,Y89,YRK5,STAGE,RK5,XHAT(1,1),
     1          IJAC,INARR,REARR) 
 ...
 61         QMAT(I,J)=STAGE(I,J)
      ...
      IF (IFLAG.NE.0) THEN
         ...
         RETURN
      ENDIF
      CALL XDOT(GETF,GETDFV,M,N,Y89,YRK5,QMAT,STAGE,XHAT(1,1),
     1          IJAC,INARR,REARR) 
      CALL DGQTB(M,M,N,QMAT,STAGE,M,BMAT,M)
      DO 65 I=1,N
 65      RKNU5(I)=BMAT(I,I)
 ...
 70   CONTINUE
C ... RK6, SOLUTION AT H ...
      CALL XDOT(GETF,GETDFV,M,N,Y1,YRK6,STAGE,RK6,XHAT(1,1),
     1          IJAC,INARR,REARR) 
 ...
 74         QMAT(I,J)=STAGE(I,J)
      ...
      IF (IFLAG.NE.0) THEN
         ...
         RETURN
      ENDIF
      CALL XDOT(GETF,GETDFV,M,N,Y1,YRK6,QMAT,STAGE,XHAT(1,1),
     1          IJAC,INARR,REARR)
      CALL DGQTB(M,M,N,QMAT,STAGE,M,BMAT,M)
      DO 77 I=1,N
 77      RKNU6(I)=BMAT(I,I)
...
C TO BE USED FOR ERROR MONITOR/STEP-SIZE SELECTION
      CALL XDOT(GETF,GETDFV,M,N,Y,YRK7,X,RK2,XHAT(1,1),
     1          IJAC,INARR,REARR) 
      ...
      IF (IFLAG.NE.0) THEN
         ...
         RETURN
      ENDIF
      CALL XDOT(GETF,GETDFV,M,N,Y,YRK7,X,STAGE,XHAT(1,1),
     1          IJAC,INARR,REARR)
      CALL DGQTB(M,M,N,X,STAGE,M,BMAT,M)
      DO 83 I=1,N
 83      RKNU2(I)=BMAT(I,I)
      DO 92 J=1,N 
         IF (IHOWLE.EQ.0.AND.IFXDPF.EQ.0.AND.(IERRC.EQ.1.OR.
     ...
     *       IERRC.EQ.21.OR.IERRC.EQ.210)) THEN
            DO 90 I=1,M
             XHAT(I,J)=X0(I,J)+H*(BHAT(1)*RK1(I,J)+
     1       BHAT(3)*RK3(I,J)+BHAT(4)*RK4(I,J)+
     2       BHAT(5)*RK5(I,J)+BHAT(6)*RK6(I,J)+BHAT(7)*RK2(I,J))
 90         CONTINUE
         ENDIF
 92   CONTINUE
     ...
     *    IERRC.EQ.21.OR.IERRC.EQ.210)) THEN
      ...
      RETURN
      END
CCCCC END OF HRK5NL CCCCC
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C
C*****Subroutine name: PQ38NL
C
C*****Purpose
C
C performs the one step integration of y'=f(y)
...
C*****Date: May 29, 2003
C
CCCCCC PQ38NL CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      SUBROUTINE PQ38NL(GETF,GETDFV,M,N,T,H,HOLD,Y0,X0,RLYAP,TOLT,
     *          TOLQ,TOLL,Q,QHAT,BMAT,RK1,RK2,RK3,RK4,STAGE,
     *          RNU,RNUHAT,RKNU1,RKNU2,RKNU3,RKNU4,
     *          ARK,BHAT,
     *          Y,YHAT,YRK1,YRK2,YRK3,YRK4,YRK5,Y1,Y13,Y23,
     *          IPAR,NREJ,IFLAG,INARR,REARR)
      ...
      DOUBLE PRECISION ARK(7,7), BHAT(7)
      DOUBLE PRECISION Y(M),YHAT(M),YRK1(M),YRK2(M),YRK3(M)
      DOUBLE PRECISION YRK4(M), YRK5(M), Y1(M),Y13(M),Y23(M)
...
CCCCCCCCCC INTEGRATOR FOR TRAJECTORY, AND Q-FACTOR
C OF TRANSITIONS.  RETURNS ALSO UPDATE FOR LES
      CALL PRK4NL(GETF,GETDFV,PREVIO,M,N,IFIRST,IWHAT,IJAC,
     *        IFXDPF,IHOWLE,IERRC,H,Y0,X0,
     *        Q,QHAT,BMAT,RK1,RK2,RK3,RK4,STAGE,
...
C         RLYAP(I)=((T-TFIRST)*SCALE)*RLYAP(I)+SCALE*RNUHAT(I)
 ...
 32   CONTINUE
      ...
      RETURN
      END
CCCCC END OF PQ38NL CCCCC
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C
C*****Subroutine name: PRK4NL
C
C*****Purpose
C
C integrates on one step the solution of x'=f(x).  If needed,
...
C*****Date: April 28, 2004
C
CCCCCC PRK4NL CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      SUBROUTINE PRK4NL(GETF,GETDFV,PREVIO,M,N,IFIRST,IWHAT,IJAC,
     *        IFXDPF,IHOWLE,IERRC,H,Y0,X0,
     *        Q,QHAT,BMAT,RK1,RK2,RK3,RK4,STAGE,
     *        RNU,RNUHAT,RKNU1,RKNU2,RKNU3,RKNU4,ARK,BHAT,
     *        Y,YHAT,YRK1,YRK2,YRK3,YRK4,YRK5,Y1,Y13,Y23,
...
C      IF (IFIRST.EQ.1) THEN
      IF ((IFIRST.EQ.1).OR.(IFXDPF.EQ.1)) THEN
         ...
         GOTO 7
      END IF
C IF NOT FAILED PREVIOUS STEP, YRK1 IS IN YRK5
C OTHERWISE, IT IS IN YRK1
      IF (.NOT.PREVIO) THEN
          DO 5 I=1,M 
 5           YRK1(I)=YRK5(I)
...
C ... RK5, AND COMPARISON SOLUTION YHAT ...
C TO BE USED FOR ERROR MONITOR/STEP-SIZE SELECTION
      CALL GETF(M,Y,YRK5,INARR,REARR) 
      DO 20 I=1,M 
         YHAT(I)=YHAT(I)+H*(BHAT(1)*YRK1(I)+
     1       BHAT(2)*YRK2(I)+BHAT(3)*YRK3(I)+BHAT(5)*YRK5(I))
 20   CONTINUE
C NOW DO IT FOR THE TRANSITION MATRIX (IF IWHAT=0)
...
C THE RNU'S ARE INITIALLY AT 0, SINCE WE TAKE ONE STEP OF INTEGRAL
 ...
 23   CONTINUE
C FORM RK1, RK2, RK3, RK4 AND RK5 
C FORM ALSO THE STAGE VALUES AT H/3, 2H/3 AND H 
C (THE LATTER IS NOT THE SOLUTION VALUE, BUT JUST THE STAGE VALUE)
C ... RK1 AND U13 ...
C VERY FIRST CALL NEED RK1
C IF FAILED PREVIOUS STEP, HAVE RK1 IN RK1
...
CC **** SEE IF CAN ECONOMIZE HERE ****
 ...
 29      RKNU1(I)=BMAT(I,I)
      ...
      ENDIF
C ... RK2 AND U23 ...
      CALL QDOT(GETF,GETDFV,M,N,Y13,YRK2,BMAT,STAGE,RK2,QHAT(1,1),
     1          IJAC,INARR,REARR)
      DO 32 J=1,N
         DO 32 I=1,M
            STAGE(I,J) = Q(I,J) + H*(ARK(3,1)*RK1(I,J)+
 ...
 34         RKNU2(I)=BMAT(I,I)
      ...
      ENDIF
C ... RK3 AND U1 (STAGE VALUE AT H) ...
      CALL QDOT(GETF,GETDFV,M,N,Y23,YRK3,BMAT,STAGE,RK3,QHAT(1,1),
     1          IJAC,INARR,REARR)
      DO 38 J=1,N
         DO 38 I=1,M
            STAGE(I,J) = Q(I,J) + H*(ARK(4,1)*RK1(I,J)+
 ...
 41         RKNU3(I)=BMAT(I,I)
      ...
      ENDIF
C ... RK4, SOLUTION AT H ...
      CALL QDOT(GETF,GETDFV,M,N,Y1,YRK4,BMAT,STAGE,RK4,QHAT(1,1),
     1          IJAC,INARR,REARR)
      IF (IHOWLE.EQ.0) THEN
         DO 43 I=1,N
 43         RKNU4(I)=BMAT(I,I)
...
C TO BE USED FOR ERROR MONITOR/STEP-SIZE SELECTION
      ...
      IF (((IERRC.EQ.2).OR.(IERRC.EQ.20).OR.(IERRC.EQ.21).OR.
     *    (IERRC.EQ.210)).AND.IFXDPF.EQ.0) THEN
         CALL QDOT(GETF,GETDFV,M,N,Y,YRK5,BMAT,Q,RK4,QHAT(1,1),
     1          IJAC,INARR,REARR)
        ELSE
C FORM Jacobian*Q
         DO 49 J=1,N
...
C 48            STAGE(I,J) = ZERO
            IF (IJAC.EQ.0) THEN
               CALL OURDFV(GETF,M,Y,YRK5,RK4(1,J),STAGE(1,J),
     1                     INARR,REARR) 
              ELSE
               CALL GETDFV(M,Y,RK4(1,J),STAGE(1,J),INARR,REARR) 
            ENDIF
 49      CONTINUE
C GET diagonal of Q^T*(Jacobian*Q)
         CALL DGQTB(M,M,N,Q,STAGE,M,BMAT,M)
 ...
 55   CONTINUE
     ...
     *    IERRC.EQ.21.OR.IERRC.EQ.210)) THEN
      ...
      RETURN
      END
CCCCC END OF PRK4NL CCCCC
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C
C*****Subroutine name: HQ38NL
C
C*****Purpose
C
C performs the one step integration of the system x'=f(x)
...
C          step size.  In the latter case, error control is 
C          performed on the trajectory, or on the Lyapunov exponents,
C          or on the Q-factor, depending on IPAR(9).
C
C       This is for "large" problems, whereby no explicit Jacobian
C          is ever formed.  Code uses routine "GETDFV" if user supplied
C          it, or the internal routine "ourdfv" depending on IJAC.
C
C*****Author: L Dieci 
C*****Date: April 29, 2004
C
CCCCCC HQ38NL CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      SUBROUTINE HQ38NL(GETF,GETDFV,M,N,T,H,HOLD,Y0,X0,RLYAP,TOLT,
     *          TOLQ,TOLL,X,XHAT,QMAT,BMAT,RK1,RK2,RK3,RK4,STAGE,
     *          RNU,RNUHAT,RKNU1,RKNU2,RKNU3,RKNU4,
     *          ARK,BHAT,
     *          Y,YHAT,YRK1,YRK2,YRK3,YRK4,YRK5,Y1,Y13,Y23,
     *          IPAR,NREJ,IFLAG,INARR,REARR)
...
CCCCCCCCCC INTEGRATOR FOR TRAJECTORY, AND Q-FACTOR
C OF TRANSITIONS.  RETURNS ALSO UPDATE FOR LES
      CALL HRK4NL(GETF,GETDFV,PREVIO,M,N,IFIRST,IWHAT,IJAC,
     *        IFXDPF,IHOWLE,IERRC,H,Y0,X0,
     *        X,XHAT,QMAT,BMAT,RK1,RK2,RK3,RK4,STAGE,
     *        RNU,RNUHAT,RKNU1,RKNU2,RKNU3,RKNU4,ARK,BHAT,
     *        Y,YHAT,YRK1,YRK2,YRK3,YRK4,YRK5, Y1,Y13,Y23,
     *        IFLAG,INARR,REARR)
      IF (IFIRST.EQ.1) IFIRST=0
...
C         RLYAP(I)=((T-TFIRST)*SCALE)*RLYAP(I)+SCALE*RNUHAT(I)
 ...
 32   CONTINUE
      ...
      RETURN
      END
CCCCC END OF HQ38NL CCCCC
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C
C*****Subroutine name: HRK4NL
C
C*****Purpose
C
C Integrates on one step the solution of x'=f(x).  If needed,
...
C*****Date: April 29, 2004
C
CCCCCC HRK4NL CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      SUBROUTINE HRK4NL(GETF,GETDFV,PREVIO,M,N,IFIRST,IWHAT,IJAC,
     *        IFXDPF,IHOWLE,IERRC,H,Y0,X0,
     *        X,XHAT,QMAT,BMAT,RK1,RK2,RK3,RK4,STAGE,
     *        RNU,RNUHAT,RKNU1,RKNU2,RKNU3,RKNU4,ARK,BHAT,
     *        Y,YHAT,YRK1,YRK2,YRK3,YRK4,YRK5,Y1,Y13,Y23,
...
C      IF (IFIRST.EQ.1) THEN
      IF ((IFIRST.EQ.1).OR.(IFXDPF.EQ.1)) THEN
         ...
         GOTO 7
      END IF
C IF NOT FAILED PREVIOUS STEP, YRK IS IN YRK5
C OTHERWISE, IT IS IN YRK1
      IF (.NOT.PREVIO) THEN
          DO 5 I=1,M 
 5           YRK1(I)=YRK5(I)
...
C ... RK4, SOLUTION AT H ...
 ...
 19   CONTINUE
      IF (IFXDPF.EQ.1) GOTO 22
C ... RK5, AND COMPARISON SOLUTION XHAT ...
C TO BE USED FOR ERROR MONITOR/STEP-SIZE SELECTION
      CALL GETF(M,Y,YRK5,INARR,REARR) 
      DO 20 I=1,M 
         YHAT(I)=YHAT(I)+H*(BHAT(1)*YRK1(I)+
...
C OTHERWISE, HAVE RK1 IN RK1 
 ...
 30   CONTINUE
C ... RK2 AND U23 ...
      CALL XDOT(GETF,GETDFV,M,N,Y13,YRK2,STAGE,RK2,XHAT(1,1),
     1          IJAC,INARR,REARR) 
      IF (IHOWLE.EQ.1) GOTO 36
      DO 32 J=1,N
         DO 32 I=1,M
 32         QMAT(I,J)=STAGE(I,J)
      ...
      IF (IFLAG.NE.0) THEN
         ...
         RETURN
      ENDIF
      CALL XDOT(GETF,GETDFV,M,N,Y13,YRK2,QMAT,STAGE,XHAT(1,1),
     1          IJAC,INARR,REARR) 
      CALL DGQTB(M,M,N,QMAT,STAGE,M,BMAT,M)
      DO 34 I=1,N
 34      RKNU2(I)=BMAT(I,I)
 ...
 40   CONTINUE
C ... RK3 AND U1 (STAGE VALUE AT H) ...
      CALL XDOT(GETF,GETDFV,M,N,Y23,YRK3,STAGE,RK3,XHAT(1,1),
     1          IJAC,INARR,REARR) 
      IF (IHOWLE.EQ.1) GOTO 47
      DO 41 J=1,N
         DO 41 I=1,M
 41         QMAT(I,J)=STAGE(I,J)
      ...
      IF (IFLAG.NE.0) THEN
         ...
         RETURN
      ENDIF
      CALL XDOT(GETF,GETDFV,M,N,Y23,YRK3,QMAT,STAGE,XHAT(1,1),
     1          IJAC,INARR,REARR) 
      CALL DGQTB(M,M,N,QMAT,STAGE,M,BMAT,M)
      DO 43 I=1,N
 43      RKNU3(I)=BMAT(I,I)
 ...
 50   CONTINUE
C ... RK4, SOLUTION AT H ...
      CALL XDOT(GETF,GETDFV,M,N,Y1,YRK4,STAGE,RK4,XHAT(1,1),
     1          IJAC,INARR,REARR) 
      IF (IHOWLE.EQ.1) GOTO 56
      DO 52 J=1,N
         DO 52 I=1,M
 52         QMAT(I,J)=STAGE(I,J)
      ...
      IF (IFLAG.NE.0) THEN
         ...
         RETURN
      ENDIF
      CALL XDOT(GETF,GETDFV,M,N,Y1,YRK4,QMAT,STAGE,XHAT(1,1),
     1          IJAC,INARR,REARR) 
      CALL DGQTB(M,M,N,QMAT,STAGE,M,BMAT,M)
      DO 55 I=1,N
 55      RKNU4(I)=BMAT(I,I)
...
C TO BE USED FOR ERROR MONITOR/STEP-SIZE SELECTION
      CALL XDOT(GETF,GETDFV,M,N,Y,YRK5,X,RK4,XHAT(1,1),
     1          IJAC,INARR,REARR) 
      CALL MODGRS(M,N,X,M,BMAT,IFLAG) 
      IF (IFLAG.NE.0) THEN
         IFLAG=10
         RETURN
      ENDIF
      CALL XDOT(GETF,GETDFV,M,N,Y,YRK5,X,STAGE,XHAT(1,1),
     1          IJAC,INARR,REARR)
      CALL DGQTB(M,M,N,X,STAGE,M,BMAT,M)
      DO 65 I=1,N
 65      RKNU4(I)=BMAT(I,I)
 ...
 74   CONTINUE
     ...
     *    IERRC.EQ.21.OR.IERRC.EQ.210)) THEN
      ...
      RETURN
      END
CCCCC END OF HRK4NL CCCCC
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C
C*****Subroutine name: XDPNL
C
C*****Purpose
C
...
C*****Date: May 15, 2004
C
CCCCCC XDPNL CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      SUBROUTINE XDPNL(GETF,GETDFV,M,N,T,H,HOLD,Y0,X0,RLYAP,TOLT,
     *          TOLL,X,XHAT,RK1,RK2,RK3,RK4,RK5,RK6,STAGE,  
     *          ARK,BHAT,Y,YHAT,YRK1,YRK2,YRK3,YRK4,YRK5,
     *          YRK6,YRK7,Y1,Y15,Y03,Y45,Y89,
     *          IFXDPF,IERRC,NREJ,IFLAG,INARR,REARR)
      ...
      LOGICAL PREVIO, LAST
      INTEGER INTDIR, IFIRST, IWHAT, IJAC, I, J
      DOUBLE PRECISION HNEW, HMIN, HNEXT, TFIRST
      DOUBLE PRECISION ONE, ZERO, EXP1,EXP2
      DOUBLE PRECISION HINCR, HSFTY, HDRSTC,TEPS
      DOUBLE PRECISION SCALE, RTOL, ERR
...
C IT IS .TRUE. IF PREVIOUS STEP WAS A FAILURE
 2    CONTINUE
CCCCCCCCCC INTEGRATE FOR X: RETURNS X AND XHAT
      CALL XRK5NL(GETF,GETDFV,PREVIO,M,N,IFIRST,IWHAT,IJAC,
     *        H,Y0,X0,X,XHAT,RK1,RK2,RK3,RK4,RK5,RK6,
     *        STAGE,ARK,BHAT,
     *        Y,YHAT,YRK1,YRK2,YRK3,YRK4,YRK5,YRK6,YRK7,
...
C         RLYAP(I)=((T-TFIRST)*SCALE)*RLYAP(I)+SCALE*STAGE(I,I)
 ...
 32   CONTINUE
      ...
      RETURN
      END
CCCCC END OF XDPNL CCCCC
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C
C*****Subroutine name: XRK5NL
C
C*****Purpose
C
...
C one can compute (in XDPNL) two one-step contribution
C to the LEs.  All of these can be used for error purposes.
C 
C       This is for "large" problems, whereby no explicit Jacobian
C          is ever formed.  Code uses routine "GETDFV" if user supplied
...
C*****Date: April 29, 2004
C
CCCCCC XRK5NL CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      SUBROUTINE XRK5NL(GETF,GETDFV,PREVIO,M,N,IFIRST,IWHAT,IJAC,
     *        H,Y0,X0,X,XHAT,RK1,RK2,RK3,RK4,RK5,RK6,
     *        STAGE, ARK,BHAT,
     *        Y,YHAT,YRK1,YRK2,YRK3,YRK4,YRK5,YRK6,YRK7,
     *        Y1,Y15,Y03,Y45,Y89,IFXDPF,IERRC,INARR,REARR)
      ...
      EXTERNAL GETF,GETDFV
      LOGICAL PREVIO
      INTEGER M, N, IFIRST, IWHAT, IJAC, IFXDPF,IERRC, INARR(*)
      DOUBLE PRECISION H, Y0(M), X0(M,N), REARR(*)
      DOUBLE PRECISION X(M,N), XHAT(M,N)
      DOUBLE PRECISION RK1(M,N),RK2(M,N)
      DOUBLE PRECISION RK3(M,N),RK4(M,N)
...
C VERY FIRST CALL OR IF NOT FAILED STEP NEED RK1
C OTHERWISE, HAVE RK1 IN RK1 
C      IF ((IFIRST.EQ.1).OR.(.NOT.PREVIO)) THEN
...
C ... RK6, SOLUTION AT H ...
     ...
     1             IJAC,INARR,REARR) 
      DO 60 J=1,N
         DO 60 I=1,M
            X(I,J) = X0(I,J) + H*(ARK(7,1)*RK1(I,J)+
     1         ARK(7,3)*RK3(I,J)+ARK(7,4)*RK4(I,J)+
     2         ARK(7,5)*RK5(I,J)+ARK(7,6)*RK6(I,J))
 60   CONTINUE
...
C TO BE USED FOR ERROR MONITOR/STEP-SIZE SELECTION
 ...
 70   CONTINUE
      ...
      RETURN
      END
CCCCC END OF XRK5NL CCCCC
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C
C*****Subroutine name: X38NL
C
C*****Purpose
C
C Performs the one step integration of x'=f(x) and at the
...
C*****Date: April 28, 2004
C
CCCCCC X38NL CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      SUBROUTINE X38NL(GETF,GETDFV,M,N,T,H,HOLD,Y0,X0,RLYAP,TOLT,
     *          TOLL,X,XHAT,RK1,RK2,RK3,RK4,STAGE,ARK,BHAT,
     *          Y,YHAT,YRK1,YRK2,YRK3,YRK4,YRK5,Y1,Y13,Y23,
     *          IFXDPF,IERRC,NREJ,IFLAG,INARR,REARR)
...
C IT IS .TRUE. IF PREVIOUS STEP WAS A FAILURE
 2    CONTINUE
CCCCCCCCCC INTEGRATE FOR X: RETURNS X AND XHAT
      CALL XRK4NL(GETF,GETDFV,PREVIO,M,N,IFIRST,IWHAT,IJAC,
     *        H,Y0,X0,X,XHAT,RK1,RK2,RK3,RK4,STAGE,ARK,BHAT,
     *        Y,YHAT,YRK1,YRK2,YRK3,YRK4,YRK5,Y1,Y13,Y23,
...
C         RLYAP(I)=((T-TFIRST)*SCALE)*RLYAP(I)+SCALE*STAGE(I,I)
 ...
 32   CONTINUE
      ...
      RETURN
      END
CCCCC END OF X38NL CCCCC
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C
C*****Subroutine name: XRK4NL
C
C*****Purpose
C
C integrates on one step the solution of x'=f(x).
...
C*****Date: April 29, 2004
C
CCCCCC XRK4NL CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      SUBROUTINE XRK4NL(GETF,GETDFV,PREVIO,M,N,IFIRST,IWHAT,IJAC,
     *        H,Y0,X0,X,XHAT,RK1,RK2,RK3,RK4,STAGE,ARK,BHAT,Y,YHAT,
     *        YRK1,YRK2,YRK3,YRK4,YRK5,Y1,Y13,Y23,IFXDPF,IERRC,
     *        INARR,REARR)
...
CCCCCCC NERRNU CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      SUBROUTINE NERRNU(N,RNU,RNUHAT,ERR,TOLL,ONE,ZERO)

      IMPLICIT NONE

      INTEGER N
      DOUBLE PRECISION RNU(N), RNUHAT(N), TOLL(N), ERR
      DOUBLE PRECISION ONE, ZERO
